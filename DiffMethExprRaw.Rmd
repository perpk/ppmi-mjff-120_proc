...
```{r}
rm(list = setdiff(ls(), c("targets", "design")))
gc(full = TRUE)
```
...
```{r}
rg_set <- readRDS("rg_set_autosomal.rds")
...
rm(list = setdiff(ls(), c("rg_set", "methyl_set", "targets", "design")))
gc(full = TRUE)
```
...
### 12. Remove Cross-Reactive Probes and Single Nucleotide  Polymorphisms
...
**Remove SNPs to filter out noisy inter-individual variations without probable contribution to the phenotype**
...
snp <- ann_450k$Name[ann_450k$MASK_snp5 | ann_450k$Probe_maf > 0.01]
...
# methyl_set <- methyl_set[!rownames(methyl_set) %in% snp, ]
methyl_set_genomic <- mapToGenome(methyl_set)
methyl_set_clean <- dropLociWithSnps(methyl_set_genomic, snps = c("CpG", "SBE"), maf=0.01)
print(paste("Probes after removing single nucleotide polymorphisms:", nrow(methyl_set_clean)))
saveRDS(methyl_set_clean, "methyl_set_clean.rds")
```
...
### 13a. Retrieve cell count estimation (TODO move upwards)
...
### 13b. Remove probes on sex chromosomes

```{r}
methyl_set <- readRDS("methyl_set_clean.rds")
...
pca_df$Sex <- targets$`gender:ch1`
pca_df$Age <- targets$`age:ch1`
pca_df$DiseaseState <- targets$`disease state:ch1`

ggplot(pca_df, aes(x = PC1, y = PC2, color = Sex)) +
  geom_point(size = 3) + ggtitle("PCA by Biological Gender")
```


```{r}
cell_vars <- c("CD8T", "CD4T", "NK", "Bcell", "Mono", "Gran")

cell_melt <- melt(targets[, c("Sample_Group", cell_vars)], id.vars = "Sample_Group")
ggplot(cell_melt, aes(x = Sample_Group, y = value, fill = Sample_Group)) +
  geom_boxplot() +
  facet_wrap(~ variable, scales = "free_y") +
  ggtitle("Cell Composition by Disease Group")
```
...
colnames(design) <- gsub("Control", "", colnames(design))
colnames(design) <- gsub("`gender:ch1`Male", "Male", colnames(design))
...
![Volcano Plot - Differentially Methylated Probes](./volcano_plot.png) 
...
![Heatmap Top 10 Methylated Probes](./heatmap-top10_meth.png)
## Differentially Methylated Regions

### 24. Prepare annotation object for DMR analysis
...
dmr_input <- data.frame(
  probecpg = rownames(results),  # Probe ID
  chr = ann_450k[rownames(results), "chr"],
  pos = ann_450k[rownames(results), "pos"],
  pval = results$P.Value,
  betafc = results$logFC
)
```

### 25. Perform DMR analysis

```{r}
dmrcoutput <- dmrcate(dmr_input, 
                      lambda = 1000, 
                      C = 2)
...
```{r}
annotated_methyl_results <- read.csv("./differential_methylation_results.csv", header = T)
top_dmps <- annotated_methyl_results %>%
  filter(adj.P.Val < 0.05 & abs(logFC) > 0.1) %>%
  arrange(adj.P.Val)
beta_matrix <- readRDS("beta_matrix.rds")
ml_beta_matrix <- beta_matrix[top_dmps$ProbeID, ]
...
ml_data$Age <- targets$`age:ch1`
ml_data$Gender <- targets$`gender:ch1`
ml_data$Ethnicity <- targets$`ethnicity:ch1`
ml_data$DiseaseState <- targets$`disease state:ch1`

write.csv(ml_data, "/Volumes/Elements/pd-methyl/code/data/GSE111629/methyl_ml_data.csv", row.names = T)
```

```{r}
feature_annotation <- annotated_methyl_results %>%
  select(ProbeID, chr, pos, UCSC_RefGene_Name, UCSC_RefGene_Group, Relation_to_Island) %>%
  filter(ProbeID %in% top_dmps$ProbeID)
```

```{r}
write.csv(feature_annotation, "/Volumes/Elements/pd-methyl/code/data/GSE111629/feature_annotations.csv", row.names = F)
```

```{r}
gene_importance <-read.csv("/Volumes/Elements/pd-methyl/code/data/GSE111629/gene_importance.csv")
```
...
top_predictive_genes <- gene_importance$UCSC_RefGene_Name[1:2000]
top_predictive_genes <- unique(unlist(strsplit(top_predictive_genes, ";")))

gene_df <- bitr(top_predictive_genes, fromType="SYMBOL", toType="ENTREZID", OrgDb = org.Hs.eg.db)
ego <- enrichGO(gene = gene_df$ENTREZID, OrgDb = org.Hs.eg.db, ont="BP", pAdjustMethod = "BH", qvalueCutoff = 0.05, readable=T)
```
...
top_predictive_cpgs <- feature_annotation$ProbeID[1:1000]
all_cpgs <- rownames(ml_beta_matrix)
gsea_results <- gometh(sig.cpg = top_predictive_cpgs, all.cpg = all_cpgs, collection="GO")
```
```{r}
head(gsea_results[order(gsea_results$FDR), ])
```
```{r}
kegg_results <- gometh(sig.cpg = top_predictive_cpgs, all.cpg = all_cpgs, collection="KEGG")
```

```{r}
head(gsea_results[order(kegg_results$FDR), ])
```